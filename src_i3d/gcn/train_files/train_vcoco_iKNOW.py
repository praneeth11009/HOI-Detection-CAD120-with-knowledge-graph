...
sys.path.append(root+'/src_3D_5/gcn/')
...
sys.path.append(root+'/src_3D_5/gcn/test_files/')
...
def from_snapshot(sess):
    ...
    if args.Restore_flag == 0:
        ...
        saver_t += [var for var in tf.model_variables() if 'conv6' in var.name]
...
data_dir = root + '/data/'
dataset = data_dir + 'glove_cad120_11'
...
print('==> Load data')
wordlist_file = os.path.join(data_dir, 'list/words_cad120_11.json')
...
Trainval_GT = pkl.load( open( root + '/src_3D_5/gcn/CAD120_GT_all_subjects_training_new_wo_having_meal_with_norm_pose.pkl', "rb" ), encoding='latin1' )
Testval_GT = pkl.load( open( root + '/src_3D_5/gcn/CAD120_GT_all_subjects_testing_new_wo_having_meal_with_norm_pose.pkl', "rb" ), encoding='latin1' )
...
for epoch in range(cfg.TRAIN.EPOCHS + 1): 
    ...
    for test_batch in test_batch_ids: 
        ...
        res_log_file.write('Test\n')
        res_log_file.write('human '+str(test_label_H)+" "+str(test_H_pred)+"\n")
        res_log_file.write('object '+str(test_label_O)+" "+str(test_O_pred)+"\n")
        res_log_file.write('combined '+str(test_label_HO)+" "+str(test_HO_pred)+"\n")
