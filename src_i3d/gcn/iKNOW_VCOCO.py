...
class Triple_GCN():
    def __init__(self, is_training):
        ...
        self.human_feat = tf.placeholder(tf.float32, shape=[5, 7, 7, 1024], name = 'human_feat')
        self.object_feat = tf.placeholder(tf.float32, shape=[5, 7, 7, 1024], name = 'object_feat')
        self.combined_feat = tf.placeholder(tf.float32, shape=[5, 7, 7, 1024], name = 'combined_feat')
        ...
        self.H_pose_heat_maps= tf.placeholder(tf.float32, shape=[20, 64, 64, 1], name = 'H_poses')
        ...
        self.ivs         = tf.placeholder(tf.int32, shape=[11], name = 'idx_GT_verbs')
        ...
        self.num_verbs = 11
        self.num_classes_human = 11 ## 11
        self.num_classes_object = 13 ## 13
        ...
        if tf.__version__ == '1.1.0':
            self.blocks     = [resnet_utils.Block('block1', resnet_v1.bottleneck,[(256,   64, 1)] * 2 + [(256,   64, 2)]),
                               ...
                               resnet_utils.Block('block6', resnet_v1.bottleneck,[(2048, 512, 1)] * 3)]
        else:
            ...
            self.blocks = [resnet_v1_block('block1', base_depth=64,  num_units=3, stride=2),
                           ...
                           resnet_v1_block('block6', base_depth=512, num_units=3, stride=1)]
    ...
    def res5(self, pool5_H, pool5_O, pool5_HO, name):
        with slim.arg_scope(resnet_arg_scope(is_training=self.train)):
            ...
            fc7_HO = tf.reduce_mean(fc7_HO, axis=[1, 2]) # TensorShape([Dimension(None), Dimension(2048)])
        
        return fc7_H, fc7_O, fc7_HO
        # return fc7_O

    
    def visual_feature(self, fc7_H, fc7_O, fc7_HO, HO_box_enc, O_box_enc, name): 
        with tf.variable_scope(name) as scope:
                           
            # HO 
            vis_fc1 = slim.fully_connected(fc7_HO, 512, scope='vis_fc1')
            ...
            # concat = tf.Print(concat, [tf.shape(concat)], "The new shape is: ")
    ...
    def build_all(self):            
        ...
        model_logger = open('./model_log.txt', 'a+')
        shp = self.H_pose_heat_maps.shape
        model_logger.write("H_pose_heat_maps "+str(shp)+"\n")

        H_pose_reshaped = tf.reshape(self.H_pose_heat_maps, [1, shp[0], shp[1], shp[2], shp[3]])

        shp = self.human_feat.shape
        human_feat_reshaped = tf.reshape(self.human_feat, [1, shp[0], shp[1], shp[2], shp[3]])

        shp = self.combined_feat.shape
        combined_feat_reshaped = tf.reshape(self.combined_feat, [1, shp[0], shp[1], shp[2], shp[3]])

        shp = self.object_feat.shape
        object_feat_reshaped = tf.reshape(self.object_feat, [1, shp[0], shp[1], shp[2], shp[3]])

        pool5_heat_map = tf.nn.conv3d(tf.convert_to_tensor(H_pose_reshaped, np.float32), filter=tf.get_variable(shape=[1., 8., 8., 1., 8.], dtype=tf.float32, name='filter01'), strides=[1, 1, 4, 4, 1], padding="VALID", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_Heat_map_1")
        pool5_heat_map = tf.nn.conv3d(tf.convert_to_tensor(pool5_heat_map, np.float32), filter=tf.get_variable(shape=[1., 9., 9., 8., 64.], dtype=tf.float32, name='filter02'), strides=[1, 1, 1, 1, 1], padding="VALID", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_Heat_map_2")

        model_logger.write("pool5_heat_map "+str(pool5_heat_map.shape)+"\n")

        pool5_H = tf.nn.conv3d(tf.convert_to_tensor(human_feat_reshaped, np.float32), filter=tf.get_variable(shape=[1., 1., 1., 1024., 256.], dtype=tf.float32, name='filterH1'), strides=[1, 1, 1, 1, 1], padding="SAME", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_Human_01")
        pool5_H = tf.nn.conv3d(tf.convert_to_tensor(pool5_H, np.float32), filter=tf.get_variable(shape=[1., 1., 1., 256., 64.], dtype=tf.float32, name='filterH2'), strides=[1, 1, 1, 1, 1], padding="SAME", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_Human_02")

        pool5_HO = tf.nn.conv3d(tf.convert_to_tensor(combined_feat_reshaped, np.float32), filter=tf.get_variable(shape=[1., 1., 1., 1024., 256.], dtype=tf.float32, name='filterHO1'), strides=[1, 1, 1, 1, 1], padding="SAME", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_HumanObject_01")
        pool5_HO = tf.nn.conv3d(tf.convert_to_tensor(pool5_HO, np.float32), filter=tf.get_variable(shape=[1., 1., 1., 256., 64.], dtype=tf.float32, name='filterHO2'), strides=[1, 1, 1, 1, 1], padding="SAME", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_HumanObject_02")

        pool5_O = tf.nn.conv3d(tf.convert_to_tensor(object_feat_reshaped, np.float32), filter=tf.get_variable(shape=[1., 1., 1., 1024., 256.], dtype=tf.float32, name='filterO1'), strides=[1, 1, 1, 1, 1], padding="SAME", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_Object_01")
        pool5_O = tf.nn.conv3d(tf.convert_to_tensor(pool5_O, np.float32), filter=tf.get_variable(shape=[1., 1., 1., 256., 64.], dtype=tf.float32, name='filterO2'), strides=[1, 1, 1, 1, 1], padding="SAME", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_Object_02")
        
        model_logger.write("pool5_H "+str(pool5_H.shape)+"\n")
        model_logger.write("pool5_HO "+str(pool5_HO.shape)+"\n")
        model_logger.write("pool5_O "+str(pool5_O.shape)+"\n")

        pool5_H = tf.concat([pool5_H, pool5_heat_map], 1)
        pool5_HO = tf.concat([pool5_HO, pool5_heat_map], 1)
        pool5_O = tf.concat([pool5_O, pool5_heat_map], 1)

        model_logger.write("concat pool5_H "+str(pool5_H.shape)+"\n")
        model_logger.write("concat pool5_HO "+str(pool5_HO.shape)+"\n")
        model_logger.write("concat pool5_O "+str(pool5_O.shape)+"\n")        

        pool5_H = tf.nn.conv3d(tf.convert_to_tensor(pool5_H, np.float32), filter=tf.get_variable(shape=[1.,  4., 4., 64., 1.], dtype=tf.float32, name='filter1'), strides=[1, 1, 1, 1, 1], padding="SAME", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_Human")
        pool5_O = tf.nn.conv3d(tf.convert_to_tensor(pool5_O, np.float32), filter=tf.get_variable(shape=[1., 4., 4., 64., 1.], dtype=tf.float32, name='filter2'), strides=[1, 1, 1, 1, 1], padding="SAME", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_Object")
        pool5_HO = tf.nn.conv3d(tf.convert_to_tensor(pool5_HO, np.float32), filter=tf.get_variable(shape=[1., 4., 4., 64., 1.], dtype=tf.float32, name='filter3'), strides=[1, 1, 1, 1, 1], padding="SAME", data_format='NDHWC', dilations=[1, 1, 1, 1, 1], name="2D_conv_HumanObject")
        ...
        pool5_HO = tf.reduce_mean(pool5_HO, axis=0)

        pool5_H = tf.expand_dims(pool5_H, 0)
        pool5_O = tf.expand_dims(pool5_O, 0)
        pool5_HO = tf.expand_dims(pool5_HO, 0)

        fc7_H, fc7_O, fc7_HO = self.res5(pool5_H, pool5_O, pool5_HO, 'res5')
        # fc7_O = self.res5(pool5_O, 'res5')               
        HO_fc, H_fc, O_fc = self.visual_feature(fc7_H, fc7_O, fc7_HO, self.HO_boxes_enc, self.O_boxes_enc, 'fc_feature')
    ...
    def train_step(self, sess, blobs):
        feed_dict = {self.gt_class_H: blobs['gt_class_H'], 
                     ...
                     ,self.inputs: blobs['features'], self.H_pose_heat_maps: blobs['human_pose_heat_maps'],
    ...
    def test_step(self, sess, blobs):
        ...
        feed_dict = {self.image: blobs['image'], self.H_boxes: blobs['H_boxes'],
                     ...
                     self.H_pose_heat_maps: blobs['human_pose_heat_maps'], 
                     self.num_nonzero: blobs['num_features_nonzero'], self.ivs: blobs['iv_all']}
        feed_dict.update({self.support[i]: blobs['support'][i] for i in range(len(blobs['support']))})
    ...
    def test_image_HO(self, sess, image, blobs):
        feed_dict = {self.image: image, self.H_boxes: blobs['H_boxes'], self.O_boxes: blobs['O_boxes'], 
                     ...
                     self.inputs: blobs['features'], self.H_pose_heat_maps: blobs['human_pose_heat_maps'], 
                     self.num_nonzero: blobs['num_features_nonzero'], 
