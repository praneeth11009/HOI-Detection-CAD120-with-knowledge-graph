...
def parse_arg():
    ...
    parser.add_argument('--path', type=str, default='',
                        help='path to pretrained word embedding model') # glove pretrained .txt
...
if __name__ == '__main__':
    args = parse_arg()
    # text_file = os.path.join(data_dir, 'list', 'words_hico_vrd.json')
    # model_path = args.path
    text_file = os.path.join(data_dir, 'list', 'words_cad120_11.json')
    model_path = os.path.join(data_dir, 'text')
    if args.wv == 'glove':
        # save_file = os.path.join(data_dir, 'word_embedding_model', 'glove_hico_vrd.pkl')
        save_file = os.path.join(data_dir, 'word_embedding_model', 'glove_cad120_11.pkl')
