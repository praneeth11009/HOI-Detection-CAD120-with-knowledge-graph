...
def get_video_data(blobs):
    ...
    human_video_segment = torch.Tensor(human_video_segment).float().cuda()
    object_video_segment = torch.Tensor(object_video_segment).float().cuda()

    return human_video_segment, object_video_segment

def compute_cooccurence(blobs_multiple_train, blobs_multiple_test):
