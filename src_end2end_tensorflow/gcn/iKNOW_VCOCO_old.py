...
class Triple_GCN():
    ...
    def add_loss(self):
        with tf.variable_scope('LOSS') as scope:
            ...
            O_cross_entropy  = tf.reduce_mean(tf.multiply(tf.nn.sigmoid_cross_entropy_with_logits(labels = label_HO, logits = cls_score_O),  HO_mask))
            ...
            loss = HO_cross_entropy + 2 * H_cross_entropy + O_cross_entropy + FLAGS.sim_weight * dist_loss 
    ...
    def train_step_with_summary(self, sess, blobs, merge):
        ...
        return summary, total_loss, now_lr, sess
    ...
    def test_image_H(self, sess, image, blobs):
        feed_dict = {self.image: image, self.H_boxes: blobs['H_boxes'], self.H_num: blobs['H_num'],
                     self.H_boxes_enc:blobs['H_boxes_enc'], self.inputs: blobs['features'], 
                     self.num_nonzero: blobs['num_features_nonzero'], self.ivs: blobs['iv_all']}
        feed_dict.update({self.support[i]: blobs['support'][i] for i in range(len(blobs['support']))})
